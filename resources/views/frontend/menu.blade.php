                            ...
                            <div class="col-sm-6 col-lg-4 py-3 all {{ $product->category->slug ?? '' }}">
                                <div class="box">
                                    ...
                                    <div class="detail-box">
                                        <h5>
                                            <a href="{{ route('menu.show', $product) }}" class="text-decoration-none text-reset">{{ $product->name }}</a>
                                        </h5>
                                        <p>{{ $product->description }}</p>
                                        <div class="options">
                                            <h6>{{ number_format($product->base_price_cents) }} đ</h6>
                                            @if (($product->stock ?? 0) <= 0)
                                            ...
                                            <div class="modal-body py-5 bg-light">
                                                <div class="row g-4">
                                                    <div class="col-md-5 text-center">
                                                        <img src="{{ asset($product->image_url) }}" class="img-fluid rounded-4 shadow-sm"
                                                            style="max-height: 280px; object-fit: cover;">
                                                        <h3 class="mt-4 mb-2 fw-bold">{{ $product->name }}</h3>
                                                        <p class="text-muted mb-3">{{ $product->description }}</p>
                                                        <h4 class="text-danger fw-bold">{{ number_format($product->base_price_cents) }}đ</h4>
                                                    </div>
                                                    <div class="col-md-7">
                                                        ...
                                                        <form action="{{ route('cart.add', $product->id) }}" method="POST">
                                                                ...
                                                                @for ($i = 0; $i <= 7; $i++)
                                                                    <div class="text-start">
                                                                        <input class="btn-check" type="radio" name="spicy_level"
                                                                            id="level{{ $product->id }}_{{ $i }}" value="{{ $i }}"
                                                                            {{ $i == 3 ? 'checked' : '' }}>
                                                                        <label class="btn spicy-btn" for="level{{ $product->id }}_{{ $i }}">
                                                                            <div class="fw-bold fs-20">Cấp {{ $i }}</div>
                                                                        </label>
                                                                    </div>
                                                                    <hr>
                                                                @endfor
                                                            ...
                                                            <div class="text-center">
                                                                <button type="submit" class="btn btn-danger btn-lg px-5 py-3 rounded-pill shadow">
